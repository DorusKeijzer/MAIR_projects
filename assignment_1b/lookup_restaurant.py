...
class RestaurantLookup:
    ...
    def lookup(self, preferences: dict):
        ...
        scored_restaurants = self._score_restaurants(filtered_data, preferences)
        selected_restaurant, _ = self._select_best_restaurant(scored_restaurants)
    ...
    def _select_best_restaurant(self, scored_restaurants):
        ...
        best_matches = [restaurant for restaurant, score in scored_restaurants if score == max_score]

        # Pick one at random if there are multiple best matches
        selected_restaurant = random.choice(best_matches) if len(best_matches) > 1 else best_matches[0]

        # Store the remaining restaurants for future suggestions
        remaining_restaurants = [restaurant for restaurant, score in scored_restaurants if restaurant is not selected_restaurant]
